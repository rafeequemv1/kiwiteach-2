...
- **Diagram/Label Questions**: **STRICTLY CONCISE**.
  ...
  - DO NOT write a paragraph. Direct identification only.`,
    ...
    - **ORGANIC CHEMISTRY DEFAULTS**: Prefer **bond-line** diagrams: reactions, **(I)(II)(III)** panels, resonance, curved arrows, **?** for unknown product — not beakers/burners unless the syllabus item is lab technique. **sourceImageIndex** must match the reference page that actually shows that scheme.
    - **ANTI-CHEAT / NO FOUR-OPTION STRIP**: NEVER put **all four** MCQ structures in the **one** image with **A–D** (exam option letters). Those structures go in **JSON options** (KaTeX). **(I)(II)(III)** on the stem figure is allowed (PYQ style).
...
export const generateQuizQuestions = async (
  ...
  const visualInstruction = figureCount > 0 
    ? `[VISUAL_MANDATE]:
       ...
       ${figureBreakdown ? `- **REFERENCE_QUOTA (MANDATORY)**: ${figureBreakdown}` : ''}
       - **sourceImageIndex CONTRACT**: When multiple bitmaps are attached, **each** figure-question must use the index of the **same** page that shows the structures/reaction/graph the stem describes. Wrong index → wrong trace. Never guess 0 for every row if another attachment holds the relevant diagram.
       ...
       ` : ''}
       - **POINTER LINES (WHEN REQUIRED)**: If the stem asks to identify **specific parts drawn on the diagram** (P, Q, R, numbered loci), figurePrompt **must** require **leader lines** from each such marker to the correct site. **Do not** use **A–D** on the diagram as structure markers when A–D are the exam’s **four MCQ option labels** in JSON — use **(1)(2)(3)(4)** or **P/Q/R** on-diagram instead.
       - **ORGANIC / GOC FIGURE LAYOUT (NEET–STYLE)**: For **organic chemistry**, the main figure is usually **bond-line chemistry**: multi-step arrows, **(I)(II)(III)** panels in one row, resonance contributors, curved-arrow mechanisms, or **?** / **P** for unknown product/intermediate. **Do not** default to glassware/apparatus unless the topic is lab technique. Structures for “which is the product / most stable / correct arrows” belong as **KaTeX or \\chemfig-style text in JSON options** — the **single** rendered figure is the **stem** diagram only (not four-option strips).
       ...
       - **NO FOUR-OPTION STRIP IN THE RENDERED FIGURE**: Do **not** put all **four** MCQ answer structures into the **one** exam figure as a labeled A–D answer key. **Allowed**: (a) stem figure with **(I)(II)(III)** or reaction + **?**; (b) mechanism options drawn as **(1)–(4)** small diagrams only when the **stem** says each option shows a different arrow pattern (then options text can be minimal). **Structures for “pick the product / isomer / base”** → render in **options** strings (KaTeX), not inside the single figure asset.
       - **FIGURE PROMPT RULES**: 
         ...
         - **LABEL-TYPE vs CONTEXT-ONLY**:
           ...
           - **Multi-panel org. chem.** (stem shows “(I) (II) (III)” or compares structures): figurePrompt may include those **Roman numerals** on panels as in PYQ papers — not the same as MCQ option letters A–D.
           - **Context-only** (diagram is setup only—graph, **non-chem** apparatus, generic unlabeled pathway—and the stem does NOT ask to pick among marked parts): prefer clean line art; **organic** items usually need **some** labels (I/II/III, ?, reagents) — do not strip all chemistry labels just to satisfy “unlabeled”.
...
4. **ANTI-ANSWER / NO FOUR-OPTION ANSWER STRIP**:
   - **Forbidden**: One raster showing **all four** MCQ structures with **A–D** (exam option letters) — that mirrors the answer UI. Put those structures in **JSON option strings** (KaTeX) instead.
   - **Allowed**: NEET-style **(I)(II)(III)** rows, reaction + **?**, curved-arrow **(1)–(4)** mechanism choices **only** when the stem explicitly compares those diagrams.
