...
import ReviewerQueuePanel from './ReviewerQueuePanel';
...
type FlagsTab = 'teacher' | 'reviewer';

const OutOfSyllabusFlagsPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<FlagsTab>('teacher');
  ...
  const tabBtn = (id: FlagsTab, label: string) => (
    <button
      key={id}
      type="button"
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-zinc-50/50">
      <div className="shrink-0 border-b border-zinc-200 bg-white px-4 py-2.5 md:px-5">
        <div className="inline-flex gap-1 rounded-xl border border-zinc-200 bg-zinc-50 p-1">
          {tabBtn('teacher', 'Teacher flags')}
          {tabBtn('reviewer', 'Reviewer queue')}
        </div>
      </div>
