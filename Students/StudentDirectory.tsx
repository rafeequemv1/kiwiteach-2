...
export interface Student {
  ...
  user_id?: string; // Owned by this user
  // For UI
  avatar: string;
}
...
const StudentDirectory: React.FC = () => {
  ...
  const [schoolsAndClasses, setSchoolsAndClasses] = useState<SchoolClass[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
        ...
        // 2. Fetch Schools and School Classes from new schema
        ...
        if (schoolsRes.error) console.error(schoolsRes.error);
        if (classesRes.error) console.error(classesRes.error);

        const mappedSchools = (schoolsRes.data || []).map(s => ({
            id: s.id,
            name: s.name,
        ...
        const mappedClasses = (classesRes.data || []).map(c => ({
            ...
            name: c.name,
            type: 'class' as const,
            parent_id: c.school_id // Maps to school
        }));

        setSchoolsAndClasses([...mappedSchools, ...mappedClasses]);
  ...
  const schools = useMemo(() => schoolsAndClasses.filter(item => item.type === 'school'), [schoolsAndClasses]);
  const classesMap = useMemo(() => {
    const map = new Map<string, { name: string, parent_id: string | null }>();
    schoolsAndClasses.filter(item => item.type === 'class').forEach(c => map.set(c.id, { name: c.name, parent_id: c.parent_id }));
    return map;
  }, [schoolsAndClasses]);
  ...
  const filteredStudents = useMemo(() => {
    return students.filter(s => {
      ...
      const studentClass = s.class_id ? classesMap.get(s.class_id) : null;
      const matchesSchool = studentClass?.parent_id === selectedSchoolId;
      
      return matchesSearch && matchesSchool;
    });
  }, [students, searchQuery, selectedSchoolId, classesMap]);
  ...
  const downloadCsvTemplate = () => {
    const headers = "Name, Email, Mobile Phone, Attending Exams";
    const example = "\nJohn Doe,john@example.com,+1-555-0101,\"NEET, JEE\"";
  ...
  const handleCsvUpload = async (e: React.FormEvent) => {
    ...
    const newStudentsPayload: Omit<Student, 'id' | 'avatar'>[] = [];
  ...
  if (selectedStudent) {
    return <StudentProfile student={selectedStudent} schoolsAndClasses={schoolsAndClasses} onBack={() => setSelectedStudent(null)} onUpdate={fetchData} />;
  }
    ...
    <div className="w-full max-w-7xl mx-auto p-4 md:p-6 animate-fade-in flex flex-col h-full overflow-hidden font-sans">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 shrink-0">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-0.5 font-medium">Manage enrollments, classes, and detailed records.</p>
        </div>
        <div className="flex gap-2 items-center">
            ...
            <button onClick={() => alert("Add Student functionality to be implemented.")} className="px-4 py-2 rounded-lg bg-accent text-white font-medium text-xs hover:bg-indigo-700 transition-all shadow-md flex items-center gap-1.5"><iconify-icon icon="mdi:plus"></iconify-icon>Student</button>
        </div>
      </header>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6 shrink-0">
          ...
          <select value={selectedSchoolId} onChange={e => { setSelectedSchoolId(e.target.value); setCurrentPage(1); }} className="bg-white border border-slate-200 rounded-xl px-4 py-2 outline-none focus:border-accent font-medium text-xs text-slate-600 shadow-sm appearance-none min-w-[160px]">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
          {isLoading ? <div className="text-center py-20 text-slate-400">Loading...</div> : (
              filteredStudents.length > 0 ? (
                  viewMode === 'grid' ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 p-0.5">
                          {paginatedStudents.map(student => {
                              const sClass = student.class_id ? classesMap.get(student.class_id) : null;
                              const schoolName = sClass?.parent_id ? schoolNameMap.get(sClass.parent_id) : 'Unassigned';
                              ...
                              return (
                                  <div key={student.id} onClick={() => setSelectedStudent(student)} className={`bg-white rounded-2xl p-4 border transition-all cursor-pointer relative group flex flex-col items-center ${isSelected ? 'border-accent ring-1 ring-accent bg-accent/5 shadow-md' : 'border-slate-100 hover:border-slate-200 shadow-sm'}`}>
                                      ...
                                      <span className="text-[8px] font-semibold px-2 py-0.5 rounded-md whitespace-nowrap bg-blue-50 text-blue-500">{schoolName}</span>
                                  </div>
                              );
                          })}
                      </div>
                                  ...
                                  {paginatedStudents.map(student => {
                                      ...
                                      return (
                                          <tr key={student.id} className={`hover:bg-slate-50/50 transition-colors group cursor-pointer ${isSelected ? 'bg-accent/[0.03]' : ''}`} onClick={() => setSelectedStudent(student)}>
                                              ...
                                              <td className="px-4 py-2 text-slate-400 font-normal">{schoolName}</td>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center opacity-30"><iconify-icon icon="mdi:account-search-outline" className="w-12 h-12 mb-3"></iconify-icon><p className="text-sm font-medium">No students found</p></div>
              )
          )}
      </div>
      ...
      {isCsvModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-[2px] animate-fade-in">
              <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 border border-white">
                  <div className="flex justify-between items-center mb-5"><h2 className="text-lg font-semibold text-slate-800">Bulk Student Import</h2><button onClick={downloadCsvTemplate} className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100 flex items-center gap-1.5 hover:bg-emerald-100 transition-colors"><iconify-icon icon="mdi:download"></iconify-icon>Template</button></div>
